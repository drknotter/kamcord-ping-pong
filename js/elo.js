...
Elo.setPingPong = function(data)
{
    ...
    for( var m=0; m<matches.length; m++ )
    {
        ...
        stats[player1]['history'].push({'versus':player2, 'score': w1+"-"+w2, 'challenger': true, 'timestamp': matches[m]["timestamp"], 'date': matches[m]["date"], 'current_rank': stats[player1]['rank']})
        stats[player2]['history'].push({'versus':player1, 'score': w2+"-"+w1, 'challenger': false, 'timestamp': matches[m]["timestamp"], 'date': matches[m]["date"], 'current_rank': stats[player2]['rank']})
    }
